...
pub(crate) fn encode_headers(headers: &HeaderMap) -> Bytes {
    if headers.is_empty() {
        return Bytes::new();
    }
...
pub(crate) fn decode_headers(frame: &[u8]) -> HeaderMap {
...
pub(crate) fn encode(name: &str, headers: &HeaderMap, payload: &[u8]) -> WireMessage {
...
mod tests {
    ...
    fn empty_headers_stay_an_empty_frame() {
        let message = encode("orders", &HeaderMap::new(), b"x");
